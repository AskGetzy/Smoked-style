...
export default function CatalogPage() {
  ...
  useEffect(() => {
    if (cartCount > 0) {
      setFloatingCartMounted(true)
      const frame = window.requestAnimationFrame(() => setFloatingCartVisible(true))
      return () => window.cancelAnimationFrame(frame)
    }

    setFloatingCartVisible(false)
    const timeout = window.setTimeout(() => setFloatingCartMounted(false), 300)
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--rustic-bg)' }}>
      ...
      {floatingCartMounted && (
        <Link
          href="/cart"
          className={`catalog-floating-cart ${floatingCartVisible ? 'catalog-floating-cart-visible' : ''}`}
          aria-label={`View cart, ${cartCount} item${cartCount === 1 ? '' : 's'}, $${cartTotal.toFixed(2)}`}
        >
          <span className="catalog-floating-cart-badge">{cartCount}</span>
          <span className="catalog-floating-cart-total">${cartTotal.toFixed(2)}</span>
        </Link>
      )}
    </div>
  )
